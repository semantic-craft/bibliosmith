# 俄语模板分层随机抽检说明 / Stratified Random Spotcheck for de->zh-Hans

本文件用于俄语到简体中文项目执行全书抽检闭环，与 `template/epub_pipeline/common/references/stratified_random_spotcheck.md` 对齐。
...
- 发现与俄语特有结构相关的系统性误译（如格关系误判、体貌/运动动词方向错误、分词/副动词链硬译、否定范围偏移、术语族不一致）时，禁止仅修本样本。
