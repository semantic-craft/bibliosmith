...
目标不是“从梵语大概翻出来”，而是产出来源清楚、版本可追溯、译文可读、译注克制、术语稳定、EPUB 制作质量合格的简体中文正本书。诗歌、戏剧、叙事诗和宗教哲学文本可以共享本模板，但具体书籍必须在 `metadata/source_text_profile.md` 中声明体裁压力和译注策略。

This template handles Sanskrit source-language issues for Simplified Chinese EPUB production. It does not replace book-type profiles such as `profiles/classical-science-zh-Hans`; use that profile in addition when the work is mathematical, astronomical, diagram-heavy, table-heavy, proof-heavy, or technically dense.
...
- `SOURCE_URL`：梵语公版或授权来源 URL，例如校勘版扫描、TEI/XML、Perseus、Wikisource、Internet Archive 或其他可核查来源。
...
严禁直接在模板原目录中制作具体书籍。执行任何书籍项目前，AI 必须先把 `template/epub_pipeline/common` 与 `template/epub_pipeline/Sanskrit-to-Simplified-Chinese` 合并复制到独立书籍工程目录，例如：

`books/zh-Hans/{number}_{目标语言书名}_{目标语言作者名}/`

如果启用 profile，则在语言方向模板之后再覆盖复制 profile：

`common -> Sanskrit-to-Simplified-Chinese -> profiles/{profile-target} -> books/zh-Hans/{number}_{目标语言书名}_{目标语言作者名}/`
...
## 梵语专项规则 / Sanskrit Rules

- 原书语言文本是底本；现代英译、法译、德译等只能作参考译本。
- 必须记录所用梵语版本、编辑者、卷册、页码/行号或章节编号。
- 如果来源是扫描件，必须记录 OCR 或人工转写状态；不能把未校 OCR 当成可靠底本。
- 若同时使用 PDF 扫描、梵语转写和第二语言译本，必须写明三者分工：PDF/校勘本影像用于底本页图和最终核验；梵语转写用于检索、切分、分词链和细节校正；第二语言译本只作参考证据。
...
- 诗歌项目必须额外建立 `metadata/sanskrit_poetry_profile.md` 或在 `metadata/source_text_profile.md` 中设置 `genre: poetry`，记录格律、复合词、意象链、地名路径、神话典故、称谓语气、译注密度和中文行文策略。
- 译诗可以采用散文化分行、节奏化散文或有节制的现代汉语诗行，但必须先在文体画像中说明，不得为了押韵、对仗或“古雅”添加原文没有的意象、心理或因果。

## 必读参考文件 / Required References

- `references/sanskrit_source_notes.md`
- `references/sanskrit_source_types.md`
- `references/sanskrit_textual_criticism_policy.md`
- `references/sanskrit_reference_translation_policy.md`
- `references/sanskrit_names_transliteration_policy.md`
- `references/sanskrit_title_strategy.md`
- `references/sanskrit_to_chinese_translation_notes.md`
- `references/sanskrit_poetry_profile.md`
- `references/quality_standard.md`

## 关键产物 / Key Records

- `metadata/source_witness_manifest.md`：底本、版本、扫描/OCR/转写、witness 和编号体系。
- `source/triangulated_source_control.md` 或等效文件：当一本书同时使用扫描、梵语转写和第二语言译本时，记录三者分工和冲突处理顺序。
- `qa/textual/textual_uncertainty_log.md`：异文、残损、拟补、OCR 不确定和语法歧义记录。
- `glossary/terms.csv`：梵语原词、必要转写、中文译名和说明。
...
1. `prompts/00_orchestrator_zh_sa.md`
2. `prompts/01_ingest_clean_zh_sa.md`
3. `prompts/02_split_zh_sa.md`
4. `prompts/03_global_translation_research_zh_sa.md`
5. `prompts/04_book_specific_research_zh_sa.md`
6. `prompts/05_pretranslation_trials_zh_sa.md`
7. `prompts/06_glossary_style_zh_sa.md`
8. `prompts/07_translate_chapters_zh_sa.md`
9. `prompts/08a_chapter_post_translation_control_zh_sa.md`
10. `prompts/08_review_fidelity_zh_sa.md`
11. `prompts/09_review_readability_imagery_zh_sa.md`
12. `prompts/10_review_terminology_zh_sa.md`
13. `prompts/11_chapter_quality_gate_zh_sa.md`
14. `prompts/13_preproduction_stage1_spec_zh_sa.md`
15. `prompts/14_preproduction_stage2_sample_zh_sa.md`
16. `prompts/15_full_book_production_zh_sa.md`
17. `prompts/16a_stratified_random_spotcheck.md`
18. `prompts/16_independent_review_agents_zh_sa.md`
19. `prompts/17_revision_routing_zh_sa.md`
20. `prompts/18a_release_versioning.md`
21. `prompts/18_final_output_zh_sa.md`
22. `prompts/19_retrospective_template_update_zh_sa.md`

若启用 `classical-science-zh-Hans` profile，则在本书专项研究、术语、图表、章节译后控制和最终评审阶段插入 profile 的额外门禁。

## 硬门禁 / Hard Gates

- 公开项目没有梵语公版或授权来源证据，不得翻译；私人自用项目没有本地书源文件和 `metadata/private_use_declaration.md`，不得翻译。
