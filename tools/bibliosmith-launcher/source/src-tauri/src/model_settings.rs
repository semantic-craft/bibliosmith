...
//! never in a file, so a pay-as-you-go key and a Token Plan key for the same
//! brand stay distinct, as the providers require. The active selection (which
//! slot and model to translate with) is not secret and lives in the launcher
//! config alongside the proxy and repo settings.
//!
...
/// Keychain account for a slot. Encodes the plan so a brand's pay-as-you-go and
/// Token Plan keys are separate entries.
...
pub fn get_model_catalog() -> Result<ModelCatalog, String> {
    ...
    Ok(ModelCatalog {
        slots: views,
        active: crate::read_active_model(),
    })
}
...
mod tests {
    ...
    fn slots() -> Vec<RegistrySlot> {
        vec![
            ...
            RegistrySlot {
                ...
                provider_type: "openai-compatible".into(),
                base_url: "https://plan.example/v1".into(),
                model: "qwen3.6-flash".into(),
                key_env: "QWEN_TOKEN_PLAN_API_KEYS".into(),
            },
        ]
    }
    ...
    fn credential_env_pairs_the_slot_key_env_with_the_stored_secret() {
        let got = credential_env(&slots(), "qwen", "token-plan", |acct| {
            assert_eq!(acct, "qwen/token-plan");
            Some("tp-secret".into())
        });
        assert_eq!(
            got,
            Some(("QWEN_TOKEN_PLAN_API_KEYS".into(), "tp-secret".into()))
        );
    }

    #[test]
    fn a_brands_two_plans_resolve_to_different_key_envs() {
        let payg = credential_env(&slots(), "qwen", "payg", |_| Some("k".into())).unwrap();
        let plan = credential_env(&slots(), "qwen", "token-plan", |_| Some("k".into())).unwrap();
        assert_ne!(payg.0, plan.0);
    }
    ...
    fn the_shipped_registry_parses_and_carries_the_expected_slots() {
        let repo_root = translation_engine_repo_root().expect("repo root");
